...
from services.database.queries import fetch_active_combos, fetch_active_menu, fetch_active_offers, fetch_tables, fetch_upsell_rules
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        app.state.upsell_rules = await fetch_upsell_rules()
        print(f"[startup] Cached {len(app.state.menu)} menu items, "
              f"{len(app.state.tables)} tables, "
              f"{len(app.state.combos)} combos, "
              f"{len(app.state.offers)} offers, "
              f"{len(app.state.upsell_rules)} upsell rules from DB.")
