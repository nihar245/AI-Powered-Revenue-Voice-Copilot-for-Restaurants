  ...
  try {
    ...
    await db.query(`
      CREATE INDEX IF NOT EXISTS idx_menu_combos_score
        ON menu_combos (combo_score DESC, combo_size DESC, combo_id ASC) WHERE is_active = TRUE
    `);
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS price_change_log (
        ...
        actual_elasticity  NUMERIC(8,4),
        revenue_impact_est NUMERIC(12,2),
        created_at         TIMESTAMPTZ DEFAULT NOW(),
        applied_at         TIMESTAMPTZ,
        applied_by         INT REFERENCES users(user_id)
      )
    `);
    await db.query(`
    ...
    await db.query(`
      ...
      SELECT
        ...
        total_orders_ever,
        first_ordered_at,
        (COALESCE(total_orders_ever, 0) < 50 OR first_ordered_at > NOW() - INTERVAL '30 days') AS is_new_item
      FROM menu_items
    `);
    await db.query(`
      ...
      SELECT
        mi.item_id,
        mi.name,
        COUNT(DISTINCT oi.order_id)::int AS order_count_60d,
        (COUNT(DISTINCT oi.order_id) >= 5) AS has_enough_data
      FROM menu_items mi
      LEFT JOIN order_items oi
        ON oi.item_id = mi.item_id
        AND oi.order_id IN (
          SELECT order_id FROM orders WHERE placed_at >= NOW() - INTERVAL '60 days'
        )
      GROUP BY mi.item_id, mi.name
    `);

    // ── Feature: Ingredient-Based Food Cost ────────────────────────────────
    ...
    // Seed common ingredients with realistic INR unit costs
    await db.query(`
    ...
    // Seed recipes using name-based lookups (idempotent via ON CONFLICT)
    await db.query(`
      ...
      SELECT mi.item_id, mv.variant_id, i.ing_id, rd.qty
      FROM recipe_data rd
    ...
    // Trigger: sync food_cost when a recipe row changes
    await db.query(`
      ...
      BEGIN
        ...
        IF v_computed IS NOT NULL THEN
          UPDATE menu_variants SET food_cost = v_computed WHERE variant_id = v_variant_id;
        END IF;
        RETURN COALESCE(NEW, OLD);
      END;
      $fn$
    `);
    await db.query(`DROP TRIGGER IF EXISTS trg_recipe_food_cost ON recipes`);
    await db.query(`
    ...
    // Trigger: recompute all affected variants when ingredient unit cost changes
    await db.query(`
      ...
      $fn$
    `);
    await db.query(`DROP TRIGGER IF EXISTS trg_ingredient_cost_sync ON ingredients`);
    await db.query(`
    ...
    // Initial back-fill: set stored food_cost from computed ingredient sums
    await db.query(`
      UPDATE menu_variants mv
         SET food_cost = subq.total_cost
        FROM (
